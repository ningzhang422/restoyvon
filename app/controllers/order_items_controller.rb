	...
	def add_new_item
		...
		if OrderItem.where("order_id = ? and item_id = ?",@order_id,@item_id).length == 0
			...
			@order.order_items.each do |io|
				...
				render json: {success: true, htmltext: htmltext,item_choosen: "item_#{@item_id}", amount: helper.number_to_currency(@order.order_items.map{|oi| oi.quantity * oi.item.amount}.sum, :unit => "€") }, status: 200
			else
...
=end
	...
	def remove_item
		...
		if OrderItem.where(order_id: @order_id, item_id: @item_id).first.update_attributes({:quantity => OrderItem.where(order_id: @order_id,item_id: @item_id).first.quantity - 1}) # :quantity => -1})
			if OrderItem.where(order_id: @order_id,item_id: @item_id).first.quantity == 0
				OrderItem.where(order_id: @order_id,item_id: @item_id).first.destroy
			end
			@order.order_items.each do |io|
		...
		if OrderItem.where("order_id = ? and item_id = ? and quantity < 0",@order_id,@item_id).length > 0
			...
			if OrderItem.create({:order_id => @order_id, :item_id => @item_id, :quantity => -1, :line_id => OrderItem.where("order_id = ? and item_id = ?",@order_id,@item_id).first.line_id})
				@order.order_items.order("line_id ASC").each do |io|
					if io.quantity < 0
                        		htmltext += "<div class='col-50 tablet-25'>
        				<div class='chip' style='padding:0 0 0 12px; background:#cc0000;'>
